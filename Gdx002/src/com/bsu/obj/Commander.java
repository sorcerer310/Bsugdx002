...
public class Commander {
	...
	private void commandRoles(BsuEvent be, Role.Type rt)
		...
		for (int i = 0; i < attackRoles.size; i++) {
			...
			if (atkrs.size != 0) {
				...
				for (Role e : atkrs) {
					e.currentHp -= r.getAttack() - e.getDefend();
	...
	private Array<Role> getRolesInSkillRange(Role h) {
		...
		if (s.type == Skill.Type.f_damage || s.type == Skill.Type.f_shifhp
				|| s.type == Skill.Type.f_box || s.type == Skill.Type.p_atkbeat
				|| s.type == Skill.Type.p_damage
				|| s.type == Skill.Type.pdot_damage
				|| s.type == Skill.Type.prob_blind
				|| s.type == Skill.Type.prob_dizzy
				|| s.type == Skill.Type.prob_nude)
			checkrs = h.type == Type.HERO ? npcs : heros;
		else if (s.type == Skill.Type.f_healing
				|| s.type == Skill.Type.p_healing
				|| s.type == Skill.Type.pbuff_atk
				|| s.type == Skill.Type.pbuff_def
				|| s.type == Skill.Type.pbuff_healing
				|| s.type == Skill.Type.pbuff_hp)
